...
| Job | Purpose |
|---|---|
| Scheduled syncs | Allow users to schedule sync updates on a schedule cadence (i.e. every day, week, or month) |
| Retries | Re-invokes failed sync steps (max 3 retries) |
| Cleanup | Deletes `syncTask` rows older than 14 days |
