...
<Columns cols={2}>
  ...
  <Card title="Core Services" >
    ...
    ```typescript
    // Three midnight UTC crons in src/index.ts
    Bun.cron("0 0 * * *", () => syncNewCron(db));      // scheduled incremental syncs
    Bun.cron("0 0 * * *", () => retryFailedTasks(db));  // failed step retries
    Bun.cron("0 0 * * *", () => deleteStaleTasks());   // 14-day task cleanup
    ```
  </Card>
</Columns>
...
Stoneturner registers three `Bun.cron` jobs at **midnight UTC** in `src/index.ts`. All three respect `CRON_ENABLED=false` in `.env`.

| Job | Service | Purpose |
|---|---|---|
| Scheduled syncs | `sync-new-cron.ts` | Reads `syncPipeline` rows, triggers `syncUpdates` when `updateDate` + frequency is due |
| Retries | `retry-cron.ts` | Re-invokes failed `syncTask` steps via `step-registry.ts` (max 3 retries) |
| Cleanup | `deleteSyncTasksPriorToDate` | Deletes `syncTask` rows older than 14 days |

Users configure per-integration sync frequency (Daily / Weekly / Monthly / No Schedule) in the UI dialog (`sync-schedule-dialog.tsx`). The choice is stored in the `syncPipeline` table and drives the scheduled sync cron.

For full details on scheduling, retries, cleanup, and FAQs, see [Sync scheduling & retries](/advance-usage).
